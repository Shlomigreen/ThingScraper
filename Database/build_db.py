...
def _insert_users(users, cur) :
    ...
    for user in users.values() :
        # add new user if doesnt exist and get its id
...
def _read_sql(sql_path=SQL_CONSTRUCTION) :
    """Reads sql_path into text."""
...
def build_database(json_path, db_path=DB_FILENAME) :
    """
    Builds a database of given things, makes and users from a json file.
        :param json_path: the absolute path to json file
        :param db_path: the path to save the database. Default: 'thingiverse.db'
    """
    data = json.load(open(json_path, 'r'))

    initiate_db = not os.path.exists(db_path)
